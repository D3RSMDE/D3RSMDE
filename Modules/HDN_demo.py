...
def masked_shift_and_scale(depth_preds, depth_gt, mask_valid):
    ...
    mask_diff = (mask_valid.reshape(mask_valid.size()[:2] + (-1,)).sum(-1, keepdims=True) + 1).to(depth_gt.device)
...
def get_contexts_ds(level, mask_valid, target_shape=None):
    if mask_valid is None:
        mask_valid = torch.ones(target_shape, dtype=torch.bool)
...
def compute_hdn_loss(SSI_LOSS, depth_preds, depth_gt, mask_valid_list):
    ...
    mask_valid_list_times = mask_valid_list.sum(dim=0).to(depth_gt.device)  # the number of  contexts for each locations
    valid_locations = (mask_valid_list_times != 0).to(depth_gt.device)  # valid locations
...
def HDN_interface(depth_preds, depth_gt, mask_valid=None, level=3):
    """
    Interface for HDN loss computation.

    Args:
        depth_preds (torch.Tensor): Predicted depth maps.
        depth_gt (torch.Tensor): Ground truth depth maps.
        mask_valid (torch.Tensor): Valid pixels mask.
        level (int): Level of contexts to consider.

    Returns:
        torch.Tensor: Computed HDN loss.
    """
    SSI_LOSS = SSIMAE()  # ssi loss function

    mask_valid_list_dr = get_contexts_dr(level, depth_gt, mask_valid)  # get contexts by hdn_dr
    mask_valid_list_dp = get_contexts_dp(level, depth_gt, mask_valid)  # get contexts by hdn_dp
    mask_valid_list_ds = get_contexts_ds(level, mask_valid, depth_gt.shape)  # get contexts by hdn_ds
    ...
    return (loss_hdn_dr + loss_hdn_dp + loss_hdn_ds) / 3  # average the losses from different contexts
